...
class AIOKafkaConsumer(object):
    ...
    def start(self):
        ...
        self._fetcher = Fetcher(
            ...
            fetch_timeout=self._consumer_timeout)

        self._coordinator = AIOConsumerCoordinator(
            self._client, self._subscription, loop=self._loop,
        ...
        if self._group_id is not None:
            if self._api_version >= (0, 8, 2):
                yield from self._coordinator.ensure_coordinator_known()
    ...
    def commit(self, offsets=None):
        ...
        """
        assert self._api_version >= (0, 8, 1), 'Requires >= Kafka 0.8.1'
    ...
    def _update_fetch_positions(self, partitions):
        ...
        """
        if self._api_version >= (0, 8, 1) and self._group_id is not None:
